...
public class ConvertToRawIndexTaskGenerator extends BaseSegmentMutatingPinotTaskGenerator {
  ...
  public Map<String, String> getConfigsForTask(TableConfig tableConfig, OfflineSegmentZKMetadata offlineSegmentZKMetadata) {
    ...
    String columnsToConvert = tableConfig.getTaskConfig().getConfigsForTaskType(MinionConstants.ConvertToRawIndexTask.TASK_TYPE)
        .get(MinionConstants.ConvertToRawIndexTask.COLUMNS_TO_CONVERT_KEY);
    if (columnsToConvert != null) {
      configs.put(MinionConstants.ConvertToRawIndexTask.COLUMNS_TO_CONVERT_KEY, columnsToConvert);
    }
    return configs;
  }
  ...
  public PinotTaskConfig getScheduledSegmentTaskConfig(TableConfig tableConfig,
    ...
    if (optimizations == null || !optimizations.contains(V1Constants.MetadataKeys.Optimization.RAW_INDEX)) {
      ...
      return new PinotTaskConfig(MinionConstants.ConvertToRawIndexTask.COLUMNS_TO_CONVERT_KEY, getConfigsForTask(
          tableConfig, offlineSegmentZKMetadata));
    }
    return null;
  }
}
